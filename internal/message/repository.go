...
func (r *MessageRepository) Update(ctx context.Context, messageID int, senderID int, content string) error {
	sql := `
		UPDATE messages SET content = $1, updated_at = $2 WHERE id = $3 AND sender_id = $4;
`
	res, err := r.database.ExecContext(ctx, sql, content, time.Now(), messageID, senderID)
	if err != nil {
		return err
	}
...
func (r *MessageRepository) Delete(ctx context.Context, messageID int, senderID int) error {
...
func (r *MessageRepository) GetByID(ctx context.Context, messageID int) (*Message, error) {
	query := `SELECT id, sender_id, room_id, receiver_id, content, created_at, updated_at 
			  FROM messages WHERE id = $1`
	row := r.database.QueryRowContext(ctx, query, messageID)
...
func (r *MessageRepository) List(ctx context.Context, roomID *int, receiverID *int) ([]*Message, error) {
	query := `SELECT id, sender_id, room_id, receiver_id, content, created_at, updated_at 
				FROM messages
